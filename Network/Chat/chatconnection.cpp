...
static const int PingInterval = 120 * 1000;
...
ChatConnection::ChatConnection(QObject *parent, QString hostname, QHostInfo info, int portnum) :
    QTcpSocket(parent),iChatConnection(parent), mHostName(hostname),mHostInfo(info), mPort(portnum), mConnectState(WaitingForLobbyEntered),
...
bool ChatConnection::connectedToUser(iUser *user)
{
    return false;
}

QString ChatConnection::getAddress()
{
   return "";
}
...
QString ChatConnection::getReceiversName()
{
    return mHostName;
}

void ChatConnection::sendMessage(QString message)
{
